...
def handler(event,context):
	...
	if A[P]=='Create':
		...
		i=h.render(application_aws_account_number=W,environment_type=X);N=open(T+D+'/terraform.tfvars','w');N.write(i);N.close();z=subprocess.check_output(p.join(['cd /tmp/template-environment-configuration','rm -rf .git','git init',q%(b,Z),'git add .','git commit -m initial']),stderr=subprocess.STDOUT,shell=R).decode();C.info('Successfully retrieved parameter {}'.format(B));send(A,F,SUCCESS,G,B)
